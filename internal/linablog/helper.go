...
const (
	// recommendedHomeDir 定义放置 linablog 服务配置的默认目录.
	recommendedHomeDir = ".linablog"

	// defaultConfigName 指定了 linablog 服务的默认配置文件名.
	defaultConfigName = "linablog.yaml"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 LINABLOG，如果是 linablog，将自动转变为大写。
	viper.SetEnvPrefix("LINABLOG")
